<footer>
    <div class="max_width_theme">
        <div class="top_block">
            <div class="col">
                <p class="title">Навигация</p>
            </div>
            <div class="col">
                <p class="title">Контакты</p>
            </div>
            <a href="#up" class="btn_up">
                <p class="name">Наверх</p>
                <img src="assets/svg/btn_up.svg" alt="" class="svg">
            </a>
        </div>
        <div class="bottom_block">
            <div class="menu">
                <ul>
                    <li>
                        <a href="">Оплата и доставка</a>
                    </li>
                    <li>
                        <a href="">Акции</a>
                    </li>
                    <li>
                        <a href="">Компания</a>
                    </li>
                    <li>
                        <a href="">Работа</a>
                    </li>
                    <li>
                        <a href="">Контакты</a>
                    </li>
                </ul>
            </div>
            <div class="contacts">
                <ul>
                    <li>
                        <img src="assets/svg/bx-map.svg" alt="" class="svg">
                        г. Махачкала, улица Гагарина, 13В
                    </li>
                    <li>
                        <img src="assets/svg/bx-phone-call.svg" alt="" class="svg">
                        <a href="tel:+7 988 292-68-99">+7 988 292-68-99</a>
                    </li>
                    <li>
                        <img src="assets/svg/bx-at.svg" alt="" class="svg">
                        <a href="mailto:poryadochniy.shop@gmail.com">poryadochniy.shop@gmail.com</a>
                    </li>
                </ul>
            </div>
        </div>
        <div class="info_block">
            <a href="/" class="left_block">
                <img src="assets/img/logo.png" alt="">
                <p>2024 © <span>Все права защищены</span></p>
            </a>
            <div class="right_block">
                <a href="">Пользовательское соглашение</a>
                <a href="">Политика обработки данных</a>
                <a href="">Публичная оферта</a>
            </div>
        </div>
    </div>
    <img src="assets/svg/нижний%20элемент%201.svg" alt="" class="svg">
</footer>
<div class="bottom_panel">
    <ul>
        <li>
            <a href="">
                <img src="assets/svg/bottom_panel_home.svg" alt="" class="svg">
                <span>Главная</span>
            </a>
        </li>
        <li>
            <a >
                <img src="assets/svg/bottom_panel_catalog.svg" alt="" class="svg">
                <span>Каталог</span>
            </a>
        </li>
        <li>
            <a href="">
                <img src="assets/svg/bottom_panel_favorites.svg" alt="" class="svg">
                <span>Избранное</span>
            </a>
        </li>
        <li>
            <a data-modal="modal_cart">
                <img src="assets/svg/bottom_panel_cart.svg" alt="" class="svg">
                <span>Корзина</span>
            </a>
        </li>
        <li>
            <a href="">
                <img src="assets/svg/bottom_panel_user.svg" alt="" class="svg">
                <span>Профиль</span>
            </a>
        </li>
    </ul>
</div>
<div class="overlay"></div>
<div class="modals">
    <div class="modal_cart">
        <div class="popup_cart">
            <div class="info_block">
                <div class="top_block">
                    <p class="title">Корзина</p>
                    <a href="" class="btn_default">Очистить корзину</a>
                    <img src="assets/svg/close_modal.svg" alt="" class="svg close_modal">
                </div>
                <div class="products_block">
                    <div class="item">
                        <div class="left_block">
                            <a href="" class="img">
                                <img src="assets/img/e0a3117bcc13b795e34596cb2d5270531.png" alt="">
                            </a>
                            <div class="count_item">
                        <span class="btn_minus">
                            -
                        </span>
                                <div class="amount">
                                    <input type="text" value="1">
                                </div>
                                <span class="btn_plus">
                            +
                        </span>
                            </div>
                        </div>
                        <div class="right_block">
                            <div class="price_block">
                                <div class="discount">
                                    50%
                                </div>
                                <div class="price">
                                    145 ₽
                                </div>
                                <div class="old_price">
                                    154 ₽
                                </div>
                            </div>
                            <a href="" class="name">
                                Комод на колесах deluxe, 4 ящика, 300х385х690 мм (серый)
                            </a>
                        </div>
                        <a href="" class="delete_prod_modal_cart">
                            <img src="assets/svg/delete_prod_cart.svg" alt="" class="svg">
                        </a>
                    </div>
                    <div class="item">
                        <div class="left_block">
                            <a href="" class="img">
                                <img src="assets/img/e0a3117bcc13b795e34596cb2d5270531.png" alt="">
                            </a>
                            <div class="count_item">
                        <span class="btn_minus">
                            -
                        </span>
                                <div class="amount">
                                    <input type="text" value="1">
                                </div>
                                <span class="btn_plus">
                            +
                        </span>
                            </div>
                        </div>
                        <div class="right_block">
                            <div class="price_block">
                                <div class="discount">
                                    50%
                                </div>
                                <div class="price">
                                    145 ₽
                                </div>
                                <div class="old_price">
                                    154 ₽
                                </div>
                            </div>
                            <a href="" class="name">
                                Комод на колесах deluxe, 4 ящика, 300х385х690 мм (серый)
                            </a>
                        </div>
                        <a href="" class="delete_prod_modal_cart">
                            <img src="assets/svg/delete_prod_cart.svg" alt="" class="svg">
                        </a>
                    </div>
                    <div class="item">
                        <div class="left_block">
                            <a href="" class="img">
                                <img src="assets/img/e0a3117bcc13b795e34596cb2d5270531.png" alt="">
                            </a>
                            <div class="count_item">
                        <span class="btn_minus">
                            -
                        </span>
                                <div class="amount">
                                    <input type="text" value="1">
                                </div>
                                <span class="btn_plus">
                            +
                        </span>
                            </div>
                        </div>
                        <div class="right_block">
                            <div class="price_block">
                                <div class="discount">
                                    50%
                                </div>
                                <div class="price">
                                    145 ₽
                                </div>
                                <div class="old_price">
                                    154 ₽
                                </div>
                            </div>
                            <a href="" class="name">
                                Комод на колесах deluxe, 4 ящика, 300х385х690 мм (серый)
                            </a>
                        </div>
                        <a href="" class="delete_prod_modal_cart">
                            <img src="assets/svg/delete_prod_cart.svg" alt="" class="svg">
                        </a>
                    </div>
                    <div class="item">
                        <div class="left_block">
                            <a href="" class="img">
                                <img src="assets/img/e0a3117bcc13b795e34596cb2d5270531.png" alt="">
                            </a>
                            <div class="count_item">
                        <span class="btn_minus">
                            -
                        </span>
                                <div class="amount">
                                    <input type="text" value="1">
                                </div>
                                <span class="btn_plus">
                            +
                        </span>
                            </div>
                        </div>
                        <div class="right_block">
                            <div class="price_block">
                                <div class="discount">
                                    50%
                                </div>
                                <div class="price">
                                    145 ₽
                                </div>
                                <div class="old_price">
                                    154 ₽
                                </div>
                            </div>
                            <a href="" class="name">
                                Комод на колесах deluxe, 4 ящика, 300х385х690 мм (серый)
                            </a>
                        </div>
                        <a href="" class="delete_prod_modal_cart">
                            <img src="assets/svg/delete_prod_cart.svg" alt="" class="svg">
                        </a>
                    </div>
                    <div class="item">
                        <div class="left_block">
                            <a href="" class="img">
                                <img src="assets/img/e0a3117bcc13b795e34596cb2d5270531.png" alt="">
                            </a>
                            <div class="count_item">
                        <span class="btn_minus">
                            -
                        </span>
                                <div class="amount">
                                    <input type="text" value="1">
                                </div>
                                <span class="btn_plus">
                            +
                        </span>
                            </div>
                        </div>
                        <div class="right_block">
                            <div class="price_block">
                                <div class="discount">
                                    50%
                                </div>
                                <div class="price">
                                    145 ₽
                                </div>
                                <div class="old_price">
                                    154 ₽
                                </div>
                            </div>
                            <a href="" class="name">
                                Комод на колесах deluxe, 4 ящика, 300х385х690 мм (серый)
                            </a>
                        </div>
                        <a href="" class="delete_prod_modal_cart">
                            <img src="assets/svg/delete_prod_cart.svg" alt="" class="svg">
                        </a>
                    </div>
                    <div class="item">
                        <div class="left_block">
                            <a href="" class="img">
                                <img src="assets/img/e0a3117bcc13b795e34596cb2d5270531.png" alt="">
                            </a>
                            <div class="count_item">
                        <span class="btn_minus">
                            -
                        </span>
                                <div class="amount">
                                    <input type="text" value="1">
                                </div>
                                <span class="btn_plus">
                            +
                        </span>
                            </div>
                        </div>
                        <div class="right_block">
                            <div class="price_block">
                                <div class="discount">
                                    50%
                                </div>
                                <div class="price">
                                    145 ₽
                                </div>
                                <div class="old_price">
                                    154 ₽
                                </div>
                            </div>
                            <a href="" class="name">
                                Комод на колесах deluxe, 4 ящика, 300х385х690 мм (серый)
                            </a>
                        </div>
                        <a href="" class="delete_prod_modal_cart">
                            <img src="assets/svg/delete_prod_cart.svg" alt="" class="svg">
                        </a>
                    </div>
                    <div class="item">
                        <div class="left_block">
                            <a href="" class="img">
                                <img src="assets/img/e0a3117bcc13b795e34596cb2d5270531.png" alt="">
                            </a>
                            <div class="count_item">
                        <span class="btn_minus">
                            -
                        </span>
                                <div class="amount">
                                    <input type="text" value="1">
                                </div>
                                <span class="btn_plus">
                            +
                        </span>
                            </div>
                        </div>
                        <div class="right_block">
                            <div class="price_block">
                                <div class="discount">
                                    50%
                                </div>
                                <div class="price">
                                    145 ₽
                                </div>
                                <div class="old_price">
                                    154 ₽
                                </div>
                            </div>
                            <a href="" class="name">
                                Комод на колесах deluxe, 4 ящика, 300х385х690 мм (серый)
                            </a>
                        </div>
                        <a href="" class="delete_prod_modal_cart">
                            <img src="assets/svg/delete_prod_cart.svg" alt="" class="svg">
                        </a>
                    </div>
                </div>
            </div>
            <div class="buy_block">
                <div class="info_buy_block">
                    <div class="item">
                        <p>Товаров</p>
                        <p>(3)</p>
                    </div>
                    <div class="item">
                        <p>Скидка</p>
                        <p>26%</p>
                    </div>
                    <div class="item">
                        <p>К оплате</p>
                        <p>678 ₽</p>
                    </div>
                </div>
                <div class="bottom_btns">
                    <a href="" class="btn_green">Перейти к оформлению</a>
                    <a href="" class="btn_default">Купить в 1 клик</a>
                </div>
            </div>
        </div>
        <div class="empty_basket" style="display: none">
            <div class="top_block">
                <p class="title">Корзина</p>
                <img src="assets/svg/close_modal.svg" alt="" class="svg close_modal">
            </div>
            <div class="info_block">
                <img src="assets/svg/cart_big.svg" alt="" class="svg">
                <p class="title">В корзине пока пусто</p>
                <p class="text">Добавить товары в корзину можно через каталог или страницу товара</p>
                <a href="" class="btn_green">Перейти в каталог</a>
            </div>

        </div>
    </div>
    <div class="modal_auth">
        <div class="popup_auth">
            <div class="prev_step" style="display: none">
                <img src="assets/svg/prev_step.svg" alt="">
                <span>Назад</span>
            </div>
            <img src="assets/svg/close_modal.svg" alt="" class="svg close_modal">
            <div class="auth_content">
                <div class="auth_content_phone active">
                    <form action="" data-steps-container>
                        <div class="current_step" data-step>
                            <h4>Войти</h4>
                            <div class="auth_tabs">
                                <p class="auth_tabs_phone active" data-auth-tabs="auth_content_phone">Телефон</p>
                                <p class="auth_tabs_email" data-auth-tabs="auth_content_email">Email</p>
                            </div>
                            <input type="text" class="phone" placeholder="+7 (___) ___-__-__">
                            <a class="btn_default next_step">
                                Войти в личный кабинет
                            </a>
                            <div class="bottom_info_auth">
                                Если вы еще не зарегистрированы , то пройдите <a class="btn_reg">Регистрацию</a>
                            </div>
                        </div>
                        <div data-step style="display: none">
                            <h5>Введите код</h5>
                            <p class="info_auth_phone_text">Мы отправили вам 4-ех значный код на whatsapp. Введите код</p>
                            <input type="text" class="code_auth" placeholder="Ввести код">
                            <input type="submit" class="btn_default" value="Войти">
                            <a class="repeat_code_submit">
                                Отправить код повторно через 58 сек.
                            </a>
                            <a class="no_whatsapp next_step">
                                У меня нет whatsapp
                            </a>
                        </div>
                        <div data-step style="display: none">
                            <h5>Введите код</h5>
                            <p class="info_auth_phone_text">К вам на номер поступит звонок.<br>
                                Введите последние 4 цифры номера телефона</p>
                            <input type="text" class="code_auth" placeholder="Ввести код">
                            <input type="submit" class="btn_default" value="Войти">
                            <a class="repeat_code_submit">
                                Отправить код повторно через 58 сек.
                            </a>
                        </div>
                    </form>
                </div>
                <div class="auth_content_email">
                    <form action="" data-steps-container>
                        <div class="current_step" data-step>
                            <h4>Войти</h4>
                            <div class="auth_tabs">
                                <p class="auth_tabs_phone" data-auth-tabs="auth_content_phone">Телефон</p>
                                <p class="auth_tabs_email active" data-auth-tabs="auth_content_email">Email</p>
                            </div>
                            <input type="text" class="email" placeholder="Email">
                            <a class="btn_default next_step">
                                Войти в личный кабинет
                            </a>
                            <div class="bottom_info_auth">
                                Если вы еще не зарегистрированы , то пройдите <a class="btn_reg">Регистрацию</a>
                            </div>
                        </div>
                        <div data-step style="display: none">
                            <h5>Введите код</h5>
                            <p class="info_auth_phone_text">Мы отправили вам 4-ех значный код на почту. Введите код</p>
                            <input type="text" placeholder="Введите код">
                            <input type="submit" class="btn_default" value="Войти в личный кабинет">
                        </div>
                    </form>
                </div>
                <div class="popup_reg_content">
                    <form action="">
                        <div class="current_step">
                            <h4>Регистрация</h4>
                            <input type="text" placeholder="ФИО">
                            <input type="text" class="phone" placeholder="Телефон">
                            <input type="text" class="email" placeholder="Email">
                            <input type="password" placeholder="Пароль">
                            <input type="password" placeholder="Подтверждение пароля">
                            <p>Уже зарегистрированы? <a class="prev_step_auth">Войти в личный кабинет</a></p>
                        </div>
                        <div class="btn_reg">
                            <input type="submit" class="btn_default" value="Зарегистрироваться">
                            <input type="checkbox" name="reg" id="reg">
                            <label for="reg">
                                Я даю свое согласие <a href="">на обработку и хранение персональных данных</a>
                            </label>
                        </div>
                    </form>
                </div>
            </div>
        </div>
    </div>
    <div class="modal_success_fast_order">
        <div class="order_success">
            <p>
                Спасибо - ваша заявка отправлена - наш менеджер свяжется с вами в течении рабочего времени
            </p>
            <div class="bottom_btns">
                <a href="" class="btn_green">Перейти в каталог</a>
                <a href="" class="btn_default">Перейти к заказам</a>
            </div>
        </div>
    </div>

    <div class="modal_callback_write">
        <img src="assets/svg/close_modal.svg" alt="" class="svg close_modal">
        <div class="popup_callback_write">
            <p class="title">
                Отправить сообщение
            </p>
            <form action="">
                <div class="grid_2">
                    <input type="text" placeholder="Имя">
                    <input type="text" class="phone" placeholder="Телефон">
                </div>
                <div class="textarea">
                    <textarea name="" id="" placeholder="Сообщение"></textarea>
                </div>
                <div class="form_footer">
                    <input type="submit" class="btn_default" value="Отправить">
                    <div class="form_policy">
                        Я согласен на <a href="">обработку
                            персональных данных</a>
                    </div>
                </div>
            </form>
        </div>
    </div>

    <div class="modal_callback">
        <img src="assets/svg/close_modal.svg" alt="" class="svg close_modal">
        <div class="popup_callback">
            <p class="title">
                Поделитесь вашим опытом, оставьте предложения по улучшению или запросите дополнительную информацию
            </p>
            <p class="text">
                Заполните заявку, мы свяжимся с вами в ближайшее время
            </p>
            <form action="">
                <span>Выберите тип</span>
                <div class="select">
                    <p data-select="">Консультация <img src="assets/svg/arrow_dropdown_grey.svg" alt="" class="svg"></p>
                    <ul class="dropdown_select">
                        <li class="data-selected">Москва</li>
                        <li class="data-selected">Самара</li>
                        <li class="data-selected">Махачкала</li>
                        <li class="data-selected">Казань</li>
                    </ul>
                </div>
                <input type="text" placeholder="ФИО">
                <input type="text" placeholder="Телефон или Email">
                <textarea name="" id="" placeholder="Подробное описание"></textarea>
                <input type="submit" value="Отправить" class="btn_default">
                <span class="bottom_policy">
                Я согласен на  <a href="">обработку персональных данных</a>
            </span>
            </form>
        </div>
    </div>

    <div class="modal_city">
        <img src="assets/svg/close_modal.svg" alt="" class="svg close_modal">
        <div class="popup_city">
            <p class="title">
                Выберите город
            </p>
            <div class="search_city">
                <input type="text" class="search" placeholder="Быстрый поиск">
            </div>
            <div class="top_city">
                <a href="">Махачкала</a>
                <a href="">Дербент</a>
                <a href="">Каспийск</a>
                <a href="">Москва</a>
                <a href="">Самара</a>
            </div>
            <div class="list_city">
                <ul>
                    <li class="parent">A</li>
                    <li class="children">
                        <ul>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                        </ul>
                    </li>
                </ul>
                <ul>
                    <li class="parent">Б</li>
                    <li class="children">
                        <ul>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                        </ul>
                    </li>
                </ul>
                <ul>
                    <li class="parent">В</li>
                    <li class="children">
                        <ul>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                        </ul>
                    </li>
                </ul>
                <ul>
                    <li class="parent">Г</li>
                    <li class="children">
                        <ul>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                        </ul>
                    </li>
                </ul>
                <ul>
                    <li class="parent">З</li>
                    <li class="children">
                        <ul>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                        </ul>
                    </li>
                </ul>
                <ul>
                    <li class="parent">И</li>
                    <li class="children">
                        <ul>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                        </ul>
                    </li>
                </ul>
                <ul>
                    <li class="parent">К</li>
                    <li class="children">
                        <ul>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                        </ul>
                    </li>
                </ul>
                <ul>
                    <li class="parent">Л</li>
                    <li class="children">
                        <ul>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                        </ul>
                    </li>
                </ul>
                <ul>
                    <li class="parent">Н</li>
                    <li class="children">
                        <ul>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                        </ul>
                    </li>
                </ul>
                <ul>
                    <li class="parent">О</li>
                    <li class="children">
                        <ul>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                        </ul>
                    </li>
                </ul>
                <ul>
                    <li class="parent">П</li>
                    <li class="children">
                        <ul>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                        </ul>
                    </li>
                </ul>
                <ul>
                    <li class="parent">Р</li>
                    <li class="children">
                        <ul>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                        </ul>
                    </li>
                </ul>
                <ul>
                    <li class="parent">С</li>
                    <li class="children">
                        <ul>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                        </ul>
                    </li>
                </ul>
                <ul>
                    <li class="parent">Т</li>
                    <li class="children">
                        <ul>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                        </ul>
                    </li>
                </ul>
                <ul>
                    <li class="parent">У</li>
                    <li class="children">
                        <ul>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                        </ul>
                    </li>
                </ul>
                <ul>
                    <li class="parent">Ф</li>
                    <li class="children">
                        <ul>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                            <li><a href="">Абакан</a></li>
                        </ul>
                    </li>
                </ul>
            </div>
        </div>
    </div>
</div>
<script src="assets/js/jquery-3.7.1.min.js"></script>
<script src="assets/js/jquery-ui.min.js"></script>
<script src="assets/js/swiper-bundle.min.js"></script>
<script src="https://cdn.jsdelivr.net/npm/@fancyapps/ui@5.0/dist/fancybox/fancybox.umd.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/slick-carousel/1.9.0/slick.min.js"></script>
<script src="assets/js/scripts.js"></script>
</body>
</html>